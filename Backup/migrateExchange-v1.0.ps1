...
###########################################################################
Function findBestFitTargetMDB ( $MBXsize, $mdbArray, $userName )
{
 $tempHT = @{}
 ...
 If (!( $Script:UserMBX2DBHash.Get_item($userName) -ge 0) )
 {
  ...
  $Script:UserMBX2DBHash.Add($userName, $mdbArray[0].Name )
  ...
  ### Return the database name (with the least stored mailboxdata) to the calling function
  $mdbArray[0].Name
 }
 Else
 {
...
Function getMailboxDatabaseSizes
{
 ...
 $t = @($mdbsize | Select-Object Name, @{Name="SizeGB"; Expression={[math]::Round( (($_.Size/1GB)),2)}})
 Foreach ($db in $mdb)
 {
  If ( $t.GetValue($db) -gt $mdbSizeHT.GetValue($db)) { $mdbSizeHT.Set_Item($db, $t.get_value($db) ) ; $mdbSizeHTUpdated = $true }
 }
}
...
If (-not $failArray)
{
 ...
 If ($users2migrate)
 {
  foreach ($try in $users2migrate.getenumerator() )
  {
   ...
   $MDBSizeOption = "Unlimited"; $DBarray = $mdbsize.GetEnumerator().Name
...
If ($mdbSizeHTUpdated) { Export-Clixml -Path $MDBSizeHTdataBaseFile -Encoding UTF8 -inputobject  $mdbSizeHT}
