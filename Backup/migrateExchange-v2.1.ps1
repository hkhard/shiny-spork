<#
    .SYNOPSIS
    #####################################################################
    # Created by Kontract (c) 2012-2016, v2.03
...
Function getOldMailboxSize ( $valPair )
{
 ...
 $returnValue = $UserMailboxStats | Select-Object DisplayName, TotalItemSizeInBytes,@{Name="TotalItemSizeGB"; Expression={[math]::Round( (($_.TotalItemSizeInBytes/1GB) * 1.1),2)}}
...
function checkandEditGroupMembership ( $user )
{
 ...
 {
  If ($group.tolower().Contains($($scpDenyGroupNAme)))          ## Checks if user is a member of the webmail group
  {
...
If ( ($confirm) -and ($mbxHTupdated)) { Export-Clixml -Path $MDBdataBaseFile -Encoding UTF8 -inputobject  $mdbHT}
If ($confirm)  { $b=@{} ; foreach ($a in $mdbSizeHT.GetEnumerator() ) { $b.add($a.Name, $a.Value) } ; Export-Clixml -Path $MDBSizeHTdataBaseFile -Encoding UTF8 -inputobject $b}  ### Fulhack f�r att g�ra om dictionary object till HT
If ( ($confirm) -and ($mbx2dbUpdated)) { $b=@{} ; foreach ($a in $mdbSizeHT.GetEnumerator() ) { $b.add($a.Name, $a.Value) } ; Export-Clixml -Path $User2MDBdataBaseFile -Encoding UTF8 -InputObject $b }
