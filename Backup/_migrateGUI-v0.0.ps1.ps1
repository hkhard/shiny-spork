...
param([string] $formerForest = "korsnas.se",[switch] $fullSync, [switch]$updateTables)

#List the attributes to syncronize
# usesXML from syncUserAttributes
# \\segavsadm01\scriptd$\_lib\$scriptFileName-userDataTable.xml 
# \\segavsadm01\scriptd$\_lib\$scriptFileName-domainControllerTable.xml
...
# Include files
####################
. \\segavsadm01.corp.lan\scriptd$\_lib\logFunctions-1.1.ps1
. \\segavsadm01.corp.lan\scriptd$\_lib\migratedDB.ps1
. \\segavsadm01.corp.lan\scriptd$\_lib\ad-1.2.ps1
...
{
 [void] $dataTable.Rows.Add("DC=corp,DC=lan","CORP","segavsdc01.corp.lan", 0, $false)
}
...
$logFileName = "\\segavsadm01.corp.lan\scriptd$\_log\$scriptFileName-" + (get-date -uformat %D) + ".log"

$pathToDomainControllerTable = "\\segavsadm01\scriptd$\_lib\$scriptFileName-domainControllerTable.xml"
$pathToUserDataBase = "\\segavsadm01\scriptd$\_lib\$scriptFileName-userDataTable.xml"
...
$script:fromAddress = "Usermig-" + ($MyInvocation.MyCommand.Name).split(".")[0]+"@billerudkorsnas.com"
...
{
 LogErrorLine "Scrippt ended errors ($processErrorCount) - $pathToDomainControllerTable not updated"
...
if ($formerForest -eq "korsnas.se")
{ $targetAddressFilter = "%@toexchange.korsnas.com" }
if ($formerForest -eq "billerud.com")
{ $targetAddressFilter = "%@toexchange.billerud.com" }

$RowsToProcess = $userTable.Select("DOMAIN='dc=corp,dc=lan' AND mailNickName is not null AND msExchHomeServerName is null AND targetAddress is not null AND targetAddress like '$targetAddressFilter'")
...
if ($result -eq "OK")
{
 if ($MigrationMode.SelectedIndex -eq 0)
 ...
 if ($result -eq "OK")
 {
  ...
  $userNamesOfAccountsToBeMigrated | & '\\segavsadm01\ScriptD$\migrateExchange\migrateExchange.ps1' -IncrementalSync:$migrationTypeIncremental -autoComplete:$migrationTypeAutoComplete -confirm:$($ConfirmCheckbox.Checked)
 }
 else
 {
  LogLine "Aborting....."
