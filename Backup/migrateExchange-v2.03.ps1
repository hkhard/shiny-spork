...
If ($confirm)  { $b=@{} ; foreach ($a in $mdbSizeHT.GetEnumerator() ) { $b.add($a.Name, $a.Value) } ; Export-Clixml -Path $MDBSizeHTdataBaseFile -Encoding UTF8 -inputobject $b}  ### Fulhack för att göra om dictionary object till HT
